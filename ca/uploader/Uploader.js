...
], function (MobileLibrary, Element, FileUtil, Device, HeaderField, UploadSetItem) {
	"use strict";

	/**
	 * Constructor for a new Uploader.
	 *
	 * @class A basic implementation for uploading and downloading one or multiple files.
	 *
	 * @extends sap.ui.core.Element
	 *
	 * @constructor
	 * @public
	 * @alias com.publix.zUI5Library.ca.uploader.Uploader
	 * @ui5-metamodel This control/element also will be described in the UI5 (legacy) designtime metamodel
	 */
	var Uploader = Element.extend("com.publix.zUI5Library.ca.uploader.Uploader", {
		metadata: {
			publicMethods: [
				"uploadItem",
				"terminateItem",
		...
		oXhr.onreadystatechange = function (oEvent) {
			...
			if (this.readyState === window.XMLHttpRequest.DONE && !oHandler.aborted) {
				that.fireUploadCompleted({
					item: oItem,
					status: {
						code: oEvent.srcElement.status,
						text: oEvent.srcElement.statusText
					}
				});
			}
		};

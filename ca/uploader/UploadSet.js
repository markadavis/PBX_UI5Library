sap.ui.define([
	...
	"com/publix/zUI5Library/library",
	"com/publix/zUI5Library/ca/uploader/utilities/deepEqual",
	"com/publix/zUI5Library/ca/uploader/utilities/KeyCodes",
	"com/publix/zUI5Library/ca/uploader/Uploader",
	"com/publix/zUI5Library/ca/uploader/UploadSetItem",
	"com/publix/zUI5Library/ca/uploader/UploadSetRenderer"
], function (Control, Icon, MobileLibrary, Button, Dialog, List, MessageBox, OverflowToolbar,
			 StandardListItem, Text, ToolbarSpacer, FileUploader, PublixLibrary, deepEqual, KeyCodes, Uploader, UploadSetItem, UploadSetRenderer) {
	"use strict";
	 ...
	 * This control builds on the {@link sap.m.uploaderCollection} control, providing better handling of headers
	 ...
	 * @constructor
	 * @public
	 * @alias com.publix.zUI5Library.ca.uploader.UploadSet
	 * @ui5-metamodel This control/element also will be described in the UI5 (legacy) designtime metamodel.
	 */
	var UploadSet = Control.extend("com.publix.zUI5Library.ca.uploader.UploadSet", {
		metadata: {
			library: "com.publix.zUI5Library",
			properties: {
				/**
				 * Defines custom text for the 'List Title' of the Upload Set.
				 */
			...
			aggregations: {
				 ...
				 */
				items: {type: "com.publix.zUI5Library.ca.uploader.UploadSetItem", multiple: true, singularName: "item"},
				/**
				 * Items representing files yet to be uploaded.
				 */
				incompleteItems: {type: "com.publix.zUI5Library.ca.uploader.UploadSetItem", multiple: true, singularName: "incompleteItem"},
				/**
				 ...
				 */
				uploader: {type: "com.publix.zUI5Library.ca.uploader.Uploader", multiple: false}
			},
			events: {
				/**
				...
				beforeItemAdded: {
					parameters: {
						 ...
						 */
						item: {type: "com.publix.zUI5Library.ca.uploader.UploadSetItem"}
					},
				...
				/**
				 * This event is fired just after the confirmation dialog for 'Remove' action is confirmed.
				 */
				afterItemRemoved: {
					parameters: {
						/**
				...
				selectionChanged: {
					parameters: {
						 ...
						 */
						items: {type: "com.publix.zUI5Library.ca.uploader.UploadSetItem[]"}
					}
				}
			}
		},
		renderer: UploadSetRenderer
	});

	var UploadState = PublixLibrary.UploadState;
	...
	UploadSet.prototype.init = function () {
		this._oRb = sap.ui.getCore().getLibraryResourceBundle("com.publix.zUI5Library");
	...
	UploadSet.prototype.getToolbar = function () {
		if (!this._oToolbar) {
			...
			if (!this._oToolbar) {
				...
				if (sTitle) {
					this._oNumberOfAttachmentsTitle = new sap.m.Title({text: sTitle});
				}
	...
	UploadSet.prototype.upload = function () {
		if (!this.getUploadEnabled()) {
			jQuery.sap.log.warning("Upload is currently disabled for this upload set.");
			return;
		}
	...
	UploadSet.prototype.getDefaultFileUploader = function () {
		...
		if (!this._oFileUploader) {
			this._oFileUploader = new FileUploader(this.getId() + "-uploader", {
				...
				uploadStarted: [this._onUploadStarted, this],
				uploadProgressed: [this._onUploadProgressed, this],
				uploadCompleted: [this._onUploadCompleted, this],
	 ...
	 * @param {Uploader} oUploader Instance of <code>com.publix.zUI5Library.ca.uploader.Uploader</code> to which the default request handlers are attached.
	 * @public
	 */
	...
	UploadSet.prototype._handleClosedDeleteDialog = function (sAction) {
		if (sAction !== MessageBox.Action.OK) {
			return;
		} else if (!this.fireAfterItemRemoved({item: this._oItemToBeDeleted})) {
			return;
		}
		// this._removeItem(this._oItemToBeDeleted);
	...
	// UploadSet.prototype._removeItem = function (oItem) {
	// 	var oItemBindingContext = null;
	// 	if (oItem.hasModel()) {
	// 		oItemBindingContext = oItem.getBindingContext();
	// 	}
	// 	if (oItemBindingContext) {
	// 		var oModel = this.getModel(),
	// 			sItemsPath = this.getBinding("items").sPath,
	// 			aModelItems = oModel.getProperty(sItemsPath),
	// 			aUpdatedItems = [];

	// 		aUpdatedItems = aModelItems.filter(function(oModelItem) {
	// 			var oModelItemContext = oModelItem.getBindingContext(),
	// 				bKeep = false;
	// 			if (oModelItemContext) {
	// 				bKeep = oModelItemContext.sPath === oItemBindingContext.sPath ? false : true;
	// 			}
	// 			return bKeep;
	// 		});

	// 		this.removeItem(oItem);
	// 		oModel.setProperty(sItemsPath, aUpdatedItems);
	// 	} else {
	// 		this.removeItem(oItem);
	// 	}
	// };
